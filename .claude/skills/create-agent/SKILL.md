---
name: create-agent
description: Create a new agent in this AgentOS template with guided or direct workflow
when-to-use:
  - create a new agent
  - build me an agent
  - add an agent
...
Use the answers + the `agno-docs` MCP (configured in `.mcp.json`) to surface **3-5 concrete agent ideas** grounded in real agno toolkits. For each suggestion: a short name, a one-sentence purpose, and the toolkit(s) it would use.
...
2. **Pattern** — propose the right one with a heuristic, don't make the user choose blind:
   - **Direct tools** (like `agents/web_search.py`): default when the agent uses ≤2 toolkits, or the user explicitly named the tools.
   - **Context provider** (like `agents/code_search.py`): default when the agent queries a single information source through one `query_<thing>` tool, or you're hiding a sub-agent.
...
- Preferred: the `agno-docs` MCP server (configured in `.mcp.json`) — search for the toolkit / integration name and read the relevant page(s).
...
- **Direct tools** → mirror `agents/web_search.py`.
- **Context provider** → mirror `agents/code_search.py`.
...
Add three suggested prompts to `app/config.yaml` under `chat.quick_prompts`, keyed by the agent's `id`:
...
- **Tool not firing when expected** — the instruction prompt isn't strong enough. Tell the user; suggest tightening or running `/improve-agent` once the agent is loaded.
...
2. Suggest the next-step loops:
   - `/extend-agent` — user-driven changes (add a tool, refine the prompt, fix a bug).
   - `/improve-agent` — autonomous probe-and-harden against the agent's `INSTRUCTIONS`.

A simple agent usually takes 5-10 minutes from `/create-agent` to working. More if the user asks for custom tools or an MCP server with auth.
