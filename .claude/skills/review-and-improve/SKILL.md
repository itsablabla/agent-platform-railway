...
description: Sweep the whole repo for public-consumption readiness (docs, agents, env vars, scripts)
when-to-use:
  - review the repo
  - check the repo
  - sweep the repo
...
- New agent file on disk not yet imported in `app/main.py` (add the import + append to `agents=[...]`).
...
- Dependency edits in `pyproject.toml`.
- Anything in `db/`, `compose.yaml`, or `Dockerfile`.
- Failing eval cases or failing live agents — recommend the right follow-up skill; don't fix here.
...
- Top-level docs: `README.md`, `AGENTS.md`, `docs/*.md`, `example.env`.
- Code: `app/`, `agents/`, `db/`, `evals/`, `scripts/`.
- Configs: `compose.yaml`, `Dockerfile`, `pyproject.toml`, `railway.json`.
...
- **Agent files on disk** — what's in `agents/`?
- **Env vars actually read** — grep `os.environ`, `os.getenv`, `getenv(`, plus settings/config modules.
- **Quick prompts** — what's in `app/config.yaml` under `chat.quick_prompts`?
- **Eval cases** — what's in `evals/cases.py`?
...
|---|---|---|
| Every agent file is registered | `agents/` ↔ `app/main.py` | New agent file not imported |
...
If yes, run `python -m evals`. If any case fails, add it to "Needs your call" with `/eval-and-improve` as the recommended follow-up. If the user declines, skip this step entirely — it does not affect the rest of the report.
...
- Recommended follow-up — usually `/improve-agent` (if a live agent looked off) or `/eval-and-improve` (if evals failed).
