---
name: extend-agent
description: User-driven changes to an existing agent (add tools, refine prompt, fix bugs)
when-to-use:
  - extend the agent
  - add a tool to
  - change the agent
...
- **Pattern** — direct tools (like `agents/web_search.py`) or context provider (like `agents/code_search.py`).
...
For any change touching agno surface area — toolkit imports, knowledge config, memory flags, scheduler, sub-agent patterns — search the **`agno-docs` MCP** (configured in `.mcp.json`) before writing code. Fall back to fetching <https://docs.agno.com/llms.txt> only if the MCP is unavailable.
...
- **Add a capability**:
  ...
  - *Sub-agent / context provider* — mirror `agents/code_search.py`. The parent sees one `query_<thing>(question)` tool; the sub-agent does the work.
  - *Scheduled task* — see agno scheduler docs and the `scheduler=True` line in `app/main.py`.
...
- `agents/<slug>.py` — instructions, tools, model, memory flags, knowledge.
- `app/main.py` — only if registering a new sub-agent or changing interface wiring.
- `app/config.yaml` — add or update the agent's `quick_prompts` to exercise the new capability.
- `pyproject.toml` — only if a toolkit needs new pip deps.
...
- **Recommended next step** — run `/improve-agent` to autonomously verify the agent still does what its `INSTRUCTIONS` say it does. The change you just made widened the agent's surface area; the autonomous loop catches anything that regressed.
